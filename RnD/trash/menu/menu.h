...
struct Menu {
  ...
  struct Part:Chain<II...,ItemNav>::template Part<I> {
    ...
    // static constexpr const bool has=Base::template has<Id>||Title::template has<Id>||Body::template has<Id>;

    template<typename Id>
    std::enable_if_t<
      has<Id,Title>||has<Id,Base>,
      Menu<T,B,II...>
    > withId() {return *this;}

    template<typename Id>
    auto withId()->std::enable_if_t<has<Id,Body>,decltype(m_body.template withId<Id>())> 
      {return m_body.template withId<Id>();}
